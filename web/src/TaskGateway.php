...
class TaskGateway
{
    ...
    public function get(string $id, string $ord)
    {
        $sql = "SELECT * 
                FROM GRW_INB_ASSIGNED_ORDERS
                WHERE Picker = :Picker
                AND SalesOrderNumber = :Order";
        $stmt = $this->conn->prepare($sql);
        $stmt->bindValue(":Picker", $id, PDO::PARAM_STR);
        $stmt->bindValue(":Order", $ord, PDO::PARAM_STR);
        $stmt->execute();
    ...
    public function delete(string $id)
        {
            $sql = "DELETE FROM GRW_INB_ASSIGNED_ORDERSz
                    WHERE Picker = :id";
            $stmt = $this->conn->prepare($sql);
