...
if($searchByShipday != '' && $searchByShipday == 'null'){
    $searchQuery .= " and (pk.ShipDay IS NULL) ";
} else if($searchByShipday != '' && $searchByShipday != 'null' ){
...
while ($row = mysqli_fetch_assoc($vRecords)) {
    ...
    $data[] = array(
         "SalesOrderNumber"=>'<a href="../_line_detail/index.php?link='.$row['SalesOrderNumber'].'" class="orderRef_" rel="noopener noreferrer" target="_blank" id='.$row['SalesOrderNumber'].'>'.$row['SalesOrderNumber'].'</a>',
