...
class PickTaskSaveController
{
    public function __construct(private PickTaskSaveGateway $gateway) {

    }
    public function processRequest(string $method, ?string $so, ?string $code) : void
    {
        if($so === null OR $code === '') {
            if($method == "GET") {
