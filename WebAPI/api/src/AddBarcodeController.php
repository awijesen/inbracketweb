...
class AddBarcodeController
{
    public function __construct(private AddBarcodeGateway $gateway) {

    }
    public function processRequest(string $method, ?string $barcode) : void
    {
        if($barcode === '') {
            if($method == "GET") {
            ...
            $task = $this->gateway->get($barcode);
            if($task === false) {
                $this->respondNotFound($barcode);
                return;
            }
