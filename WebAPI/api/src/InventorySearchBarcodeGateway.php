...
class InventorySearchBarcodeGateway
{
    private PDO $conn;
    ...
    public function get(string $so): array | false
    {
        ...
        LEFT OUTER JOIN INB_PRODUCT_MASTER m on m.ProductCode=p.ProductCode
        WHERE m.Barcode=:barcode2
        UNION ALL
        SELECT
        b.ID, 
        ...
        LEFT OUTER JOIN INB_PRODUCT_MASTER m on m.ProductCode=b.ProductCode
        WHERE m.Barcode=:barcode3";

        $stmt = $this->conn->prepare($sql);
        $stmt->bindValue(":barcode1", $so, PDO::PARAM_STR);
        $stmt->bindValue(":barcode2", $so, PDO::PARAM_STR);
        $stmt->bindValue(":barcode3", $so, PDO::PARAM_STR);
        $stmt->execute();
