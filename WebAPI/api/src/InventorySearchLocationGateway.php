...
class InventorySearchLocationGateway
{
    private PDO $conn;
    ...
    public function get(string $location): array | false
    {
        $sql = "SELECT 
        p.ID as 'ID',
        p.ProductCode as 'code', 
        p.PickfaceStock as 'qty', 
        'Pickface' as 'loc',
        p.Pickface as 'stockloc',
        pm.ProductDescription as 'desc'
        FROM INB_PICKFACE_STOCK p 
        LEFT OUTER JOIN INB_PRODUCT_MASTER pm on pm.ProductCode=p.ProductCode
        WHERE p.Pickface=:Location1
        GROUP BY ID
        UNION all
        SELECT 
        a.ID as 'ID',
        ...
        a.BulkStock as 'qty', 
        'Bulk' as 'loc',
        a.BulkLocation as 'stockloc',
        pm.ProductDescription as 'desc'
        FROM INB_BULK_STOCK a 
        LEFT OUTER JOIN INB_PRODUCT_MASTER pm on pm.ProductCode=a.ProductCode
        WHERE a.BulkLocation =:Location2
        GROUP BY ID";

        $stmt = $this->conn->prepare($sql);
        $stmt->bindValue(":Location1", $location, PDO::PARAM_STR);
        $stmt->bindValue(":Location2", $location, PDO::PARAM_STR);
        $stmt->execute();
