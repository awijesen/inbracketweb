...
class StocktakeLocSearchGateway
{
    private PDO $conn;
    ...
    public function get(string $loc): array | false
    {

        $sql = "SELECT StockLocation, LocationClassification 
        FROM INB_STOCK_LOC_MAINTAIN 
        WHERE StockLocation=:StockLocation";

        $stmt = $this->conn->prepare($sql);
        $stmt->bindValue(":StockLocation", $loc, PDO::PARAM_STR);
        $stmt->execute();
