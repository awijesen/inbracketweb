...
class POLinesToBePickedGateway
{
    private PDO $conn;
    ...
    public function get(string $so): array | false
    {
        ...
        ASS.ID, 
        ASS.ProductCode,
        ASS.ProductDescription,
        sum(ASS.OrderQuantity) as 'OrderQuantity'
        FROM INB_ASSIGNED_PURCHASE_ORDERS ASS
        WHERE ASS.PurchaseOrderNumber = :PurchaseOrderNumber
        AND NOT EXISTS(SELECT ProductCode FROM INB_PURCHASE_RECEIPTS PR WHERE PR.PONumber=ASS.PurchaseOrderNumber AND PR.ProductCode=ASS.ProductCode)
        GROUP BY ASS.ProductCode";
