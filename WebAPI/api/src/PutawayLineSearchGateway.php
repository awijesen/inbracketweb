...
class PutawayLineSearchGateway
{
    private PDO $conn;
    ...
    public function get(string $po, string $plate): array | false
    {

        $sql = "SELECT
        (SELECT sum(PutawayQuantity) as 'tot' FROM INB_PURCHASE_PUTAWAY WHERE PONumber=:PO_ AND PlateNumber=:Plate_) AS 'TOT',
        pa.ID, 
        pa.PutawayLocation,
        pa.PutawayQuantity
        FROM INB_PURCHASE_PUTAWAY pa
        WHERE pa.PONumber=:PO AND pa.PlateNumber=:Plate";

        $stmt = $this->conn->prepare($sql);
        $stmt->bindValue(":PO_", $po, PDO::PARAM_STR);
        $stmt->bindValue(":Plate_", $plate, PDO::PARAM_STR);
