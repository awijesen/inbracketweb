...
class StkScannedLinesGateway
{
    ...
    public function get(string $user): array | false
    {
        ...
        m.CountLocation,
        m.CountedOn
        FROM INB_STOCKTAKE_MASTER m
        LEFT OUTER JOIN INB_PRODUCT_MASTER pm ON pm.ProductCode=m.ProductCode
        WHERE m.CountUser=:CountUser
        GROUP BY ID
        ORDER BY CountedOn DESC";

        $stmt = $this->conn->prepare($sql);
        $stmt->bindValue(":CountUser", $user, PDO::PARAM_STR);
        $stmt->execute();
