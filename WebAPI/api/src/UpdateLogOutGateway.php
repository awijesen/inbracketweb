...
class UpdateLogOutGateway
{
    ...
    public function update(string $id, string $tid) : int
    {
   
          
            $sql = "UPDATE INB_USERMASTER SET LoginChannelMobile=null 
            WHERE UserCode=:UserCode AND tenent_id=:tID";

            $stmt = $this->conn->prepare($sql);

            $stmt->bindValue(":UserCode", $id, PDO::PARAM_STR);
