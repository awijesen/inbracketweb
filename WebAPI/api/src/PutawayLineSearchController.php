...
class PutawayLineSearchController
{
    public function __construct(private PutawayLineSearchGateway $gateway) {

    }
    public function processRequest(string $method, ?string $po, ?string $plate) : void
    {
        if($plate === '' || $po === '') {
            if($method == "GET") {
            ...
            $task = $this->gateway->get($po, $plate);
            if($task === false) {
