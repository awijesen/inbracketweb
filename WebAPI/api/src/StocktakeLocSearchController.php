...
class StocktakeLocSearchController
{
    public function __construct(private StocktakeLocSearchGateway $gateway) {

    }
    public function processRequest(string $method, ?string $loc) : void
    {
        if($loc === '') {
            if($method == "GET") {
            ...
            $task = $this->gateway->get($loc);
            if($task === false) {
                $this->respondNotFound($loc);
                return;
            }
