...
class DispatchListSearchGateway
{
    private PDO $conn;
    ...
    {
        $sqlw = "SELECT SalesOrderNumber from INB_PACK_LIST_DETAIL
            where SalesOrderNumber=:SalesOrder";

        $stmtw = $this->conn->prepare($sqlw);
        $stmtw->bindValue(":SalesOrder", $so, PDO::PARAM_STR);
        $stmtw->execute();
        $dataw = $stmtw->fetch(PDO::FETCH_ASSOC);

        if (!empty($dataw)) {
            $data = array("message"=>"alreadyscanned");
            return $data;
        } else {
            ...
            $stmt->bindValue(":SalesOrder", $so, PDO::PARAM_STR);
            $stmt->execute();
