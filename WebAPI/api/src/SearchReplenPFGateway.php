...
class SearchReplenPFGateway
{
    ...
    public function get(string $pf, string $pc): array | false
    {
        $sql = "SELECT Pickface, ProductCode 
        FROM INB_PICKFACE_STOCK 
        WHERE Pickface=:Pickface AND ProductCode =:ProductCode";

        $stmt = $this->conn->prepare($sql);
        $stmt->bindValue(":Pickface", $pf, PDO::PARAM_STR);
        $stmt->bindValue(":ProductCode", $pc, PDO::PARAM_STR);
        $stmt->execute();
