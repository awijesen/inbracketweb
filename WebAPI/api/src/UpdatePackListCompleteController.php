...
class UpdatePackListCompleteController
{
    public function __construct(private UpdatePackListCompleteGateway $gateway) {

    }
    public function processRequest(string $method, ?string $id, ?string $tid, string $box, string $pal) : void
    {
        if($id === null || $tid === null) {
            if($method == "GET") {
            ...
            // }
             switch($method) {
                ...
                case "PATCH":
                    ...
                    $rows = $this->gateway->update($id, $tid, $box, $pal);
