...
class SearchPOLinesToEditTaskGateway
{
    private PDO $conn;
    ...
    public function get(string $id, string $so): array | false
    {
        ...
        R.ReceivedTimeStamp,
        R.Receiver,
        R.LastPutawayLocation
        FROM INB_PURCHASE_RECEIPTS R WHERE R.PONumber=:PONumber AND R.ProductCode=:ProductCode";

        $stmt = $this->conn->prepare($sql);
        $stmt->bindValue(":PONumber", $so, PDO::PARAM_STR);
        $stmt->bindValue(":ProductCode", $id, PDO::PARAM_STR);
        $stmt->execute();
