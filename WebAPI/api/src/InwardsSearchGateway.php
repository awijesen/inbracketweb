...
class InwardsSearchGateway
{
    ...
    public function get(string $so, string $po): array | false
    {
        $sql = "SELECT 
        po.ProductCode,
        po.ProductDescription,
        po.UOM,
        sum(distinct(po.OrderQuantity)) as 'OrderQuantity',
        (SELECT SUM(s.ReceivedQuantity) from INB_PURCHASE_RECEIPTS s where s.PONumber=po.PurchaseOrderNumber and s.ProductCode=po.ProductCode) as 'ReceivedQuantity'
        FROM INB_ASSIGNED_PURCHASE_ORDERS po
        LEFT OUTER JOIN INB_PRODUCT_MASTER pro on pro.ProductCode=po.ProductCode
        WHERE pro.Barcode=:Barcode AND po.PurchaseOrderNumber=:PurchaseOrderNumber
        GROUP BY po.ProductCode";

        $stmt = $this->conn->prepare($sql);
        $stmt->bindValue(":Barcode", $so, PDO::PARAM_STR);
        $stmt->bindValue(":PurchaseOrderNumber", $po, PDO::PARAM_STR);
        $stmt->execute();
