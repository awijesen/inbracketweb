...
class InventorySearchLocationController
{
    public function __construct(private InventorySearchLocationGateway $gateway) {

    }
    public function processRequest(string $method, ?string $location) : void
    {
        if($location === null) {
            if($method == "GET") {
            ...
            $task = $this->gateway->get($location);
            if($task === false) {
                $this->respondNotFound($location);
                return;
            }
            ...
            if(count($task) <= 0) {
             ...
             switch($method) {
                ...
                case "PATCH":
                    ...
                    break;
                case "DELETE":
                    $rows = $this->gateway->delete($location);
