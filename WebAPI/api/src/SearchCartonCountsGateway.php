...
class SearchCartonCountsGateway
{
    private PDO $conn;
    ...
    public function get(string $so): array | false
    {
        ...
        $sql = "SELECT distinct(BoxCount) as 'BoxCount', distinct(PalCount) as 'PalCount'
        FROM INB_ORDER_PICKS
        WHERE SalesOrderNumber=:sonum";

        $stmt = $this->conn->prepare($sql);
        $stmt->bindValue(":sonum", $so, PDO::PARAM_STR);
        $stmt->execute();
