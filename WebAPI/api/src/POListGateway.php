...
class POListGateway
{
    ...
    public function get(string $so): array | false
    {
        ...
        ord.VendorName,
        PR.PONumber as 'exists'
        FROM INB_ASSIGNED_PURCHASE_ORDERS ord
        LEFT OUTER JOIN INB_PURCHASE_RECEIPTS PR ON PR.PONumber=ord.PurchaseOrderNumber
        WHERE ord.Receiver=:Receiver
        GROUP BY ord.PurchaseOrderNumber";

        $stmt = $this->conn->prepare($sql);
        $stmt->bindValue(":Receiver", $so, PDO::PARAM_STR);
        $stmt->execute();
