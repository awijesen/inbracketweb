...
class SearchReplenProductGateway
{
    private PDO $conn;
    ...
    public function get(string $bloc, string $plu): array | false
    {
        $sql = "SELECT
        bk.ID,
        bk.ProductCode,
        ...
        WHERE BulkLocation = :BulkLocation AND pro.Barcode=:Barcode
        GROUP BY bk.ID";

        $stmt = $this->conn->prepare($sql);
        $stmt->bindValue(":BulkLocation", $bloc, PDO::PARAM_STR);
        $stmt->bindValue(":Barcode", $plu, PDO::PARAM_STR);
        $stmt->execute();
