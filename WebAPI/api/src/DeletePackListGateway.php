...
class DeletePackListGateway
{
    ...
    {
        $sql = "DELETE FROM INB_PACK_LIST_DETAIL WHERE packListId=:id";
        $stmt = $this->conn->prepare($sql);
        ...
        $sqle = "DELETE FROM INB_PACK_LIST WHERE packListId=:id";
        $stmte = $this->conn->prepare($sqle);

        $stmte->bindValue(":id", $id, PDO::PARAM_STR);
