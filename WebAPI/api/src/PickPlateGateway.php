...
class PickPlateGateway
{
    ...
    public function get(string $plate): array | false
    {
        ...
        r.ReceivedQuantity,
        (SELECT SUM(pa.PutawayQuantity) FROM INB_PURCHASE_PUTAWAY pa WHERE pa.PONumber=r.PONumber AND pa.ProductCode=r.ProductCode AND PlateNumber=:Plates GROUP BY pa.ProductCode) as 'PAQty'
        FROM INB_PURCHASE_RECEIPTS r
        WHERE (r.PutawayStatus IS NULL OR r.PutawayStatus = 'InProgress') AND r.PlateNumber=:Plate";

        $stmt = $this->conn->prepare($sql);
        $stmt->bindValue(":Plates", $plate, PDO::PARAM_STR);
    ...
    {
        $sql = "SELECT * FROM INB_FETCH_ORDERS_LIST 
        WHERE TaggedOrder=:TaggedOrder and StockPlate=:StockPlate";

        $stmt = $this->conn->prepare($sql);
        $stmt->bindValue(":TaggedOrder", $data["TaggedOrder"], PDO::PARAM_STR);
        ...
        if (empty($datax)) { 
            ...
            VALUES (:SalesOrderNumber, :ProductCode, :PickedBy, :PickedQuantity, :PickStatus, :ProductId, :ReasonCode, :PickedOn)";
                ...
                if($this->conn->lastInsertId() > 0) {
                    $sql = "UPDATE INB_FETCH_ORDERS_LIST 
                    SET DespatchedStatus='Staging' WHERE StockPlate=:StockPlate and TaggedOrder=:TaggedOrder";   
                $stmt = $this->conn->prepare($sql);
