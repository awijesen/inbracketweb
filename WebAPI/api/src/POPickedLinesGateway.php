...
class POPickedLinesGateway
{
    ...
    public function get(string $so): array | false
    {
        ...
        sum(R.ReceivedQuantity) as 'ReceivedQuantity',
        (SELECT SUM(S.OrderQuantity) FROM INB_ASSIGNED_PURCHASE_ORDERS S WHERE S.PurchaseOrderNumber=R.PONumber AND S.ProductCode=R.ProductCode) as 'OrderQuantity'
        FROM INB_PURCHASE_RECEIPTS R
        WHERE R.PONumber = :PONumber
        GROUP BY R.ProductCode";

        $stmt = $this->conn->prepare($sql);
        $stmt->bindValue(":PONumber", $so, PDO::PARAM_STR);
        $stmt->execute();
