...
class SearchReplenProductController
{
    public function __construct(private SearchReplenProductGateway $gateway) {

    }
    public function processRequest(string $method, ?string $bloc, ? string $plu) : void
    {
        if($bloc === null OR $plu === '') {
            if($method == "GET") {
            ...
            $task = $this->gateway->get($bloc, $plu);
            if($task === false) {
                $this->respondNotFound($bloc);
                return;
            }
