...
class PackListIDSearchController 
{
    public function __construct(private PackListIDSearchGateway $gateway) {

    }
    public function processRequest(string $method, ?string $packid) : void
    {
        if($packid === '') {
            if($method == "GET") {
            ...
            $task = $this->gateway->get($packid);
            // if($task === false) {
            //     $this->respondNotFound($packid);
            //     return;
            // }
    ...
    private function respondNotFound(string $so): void
        {
            ...
            echo json_encode(["message" => "packidNotFound"]);
        }
