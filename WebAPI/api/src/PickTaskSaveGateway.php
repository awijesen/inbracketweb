    ...
    public function create(array $data)
    {
        ...
        VALUES (:SalesOrderNumber, :ProductCode, :PickedBy, :PickedQuantity, :PickStatus, :ProductId, :ReasonCode, :PickedOn)";
            ...
            if($this->conn->lastInsertId() > 0) {
                $sql = "UPDATE INB_PICKFACE_STOCK SET PickfaceStock = PickfaceStock - :PickedQuantity WHERE ProductCode = :Code";
                $stmt = $this->conn->prepare($sql);
    
                $stmt->bindValue(":PickedQuantity", $data["PickedQuantity"], PDO::PARAM_INT);
                $stmt->bindValue(":Code", $data["ProductCode"], PDO::PARAM_STR);       
                // exit;
                $stmt->execute();
