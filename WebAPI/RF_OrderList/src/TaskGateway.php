...
class TaskGateway
{
    ...
    {
        $sql = "SELECT 
        aso.SalesOrderNumber,
        aso.OrderCustomer, 
        aso.ShipDay,
        aso.CustomFlag,
        (SELECT distinct(s.SalesOrderNumber) as 'existsstatus' FROM INB_ORDER_PICKS s WHERE s.SalesOrderNumber=aso.SalesOrderNumber) as 'status'
        FROM GRW_INB_ASSIGNED_ORDERS aso
        WHERE aso.Picker = :Picker
        GROUP BY aso.SalesOrderNumber
        ORDER BY aso.ShipDay ASC";
        
        $stmt = $this->conn->prepare($sql);
        $stmt->bindValue(":Picker", $id, PDO::PARAM_STR);
        $stmt->execute();
